    ...
    def run(self, target, tor_session, config=None):
        ...
        for path in self.paths:
            ...
            try:
                ...
                if resp:
                    ...
                    if status == 200:
                        ...
                        findings.append({
                            ...
                            'detail': f'File accessible at {url} (HTTP {status})',
                            'url': url,
                            'status_code': status
                        })
                    
                    # 403 Forbidden - might exist but protected
                    elif status == 403:
                        findings.append({
                            'check': self.name,
                            'severity': 'info',
                    ...
                    # 401 Unauthorized - might exist with auth
                    elif status == 401:
                        findings.append({
                            'check': self.name,
                            'severity': 'low',
                ...
                # Silently ignore timeouts/errors for most paths
                pass
